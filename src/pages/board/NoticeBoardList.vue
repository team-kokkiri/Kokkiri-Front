<template>
  <BoardPageLayout
      ...
      @next="goNext"
  >
    <template #write-form v-if="isAdmin">
      <ListWriteForm
          :boardTypeId="4"
...
// 상태 변수들
const noticeBoardList = ref([])         // 공지사항 게시글 목록
...
// API에서 공지사항 게시글 리스트 불러오기
const fetchNoticeBoardList = async () => {
  try {
    // 공지사항 게시판 ID는 4로 가정 (실제 백엔드 설정에 따라 조정)
    const res = await axios.get(`${API_BASE_URL}/api/boards/list/4`, {
      headers: {
    ...
    noticeBoardList.value = Array.isArray(data.boardListResDtos) ? data.boardListResDtos : []
    currentPage.value = data.currentPage + 1
    totalPages.value = data.totalPages
    totalElements.value = data.totalElements
    isLastPage.value = data.isLast
    console.log('공지사항 게시글 API 응답:', data)

  } catch (err) {
    console.error('공지사항 게시글 목록 가져오기 실패:', err)
  }
}

// 페이지 진입 시 공지사항 게시글 호출
onMounted(async () => {
  await fetchNoticeBoardList()
})
...
function goToDetail({ id, boardId }) {
  ...
  router.push(`/main-page/notice/${targetId}`)
}
